...
const ui = {
  dist: $('distance'), ban: $('bananas'), combo: $('combo'), comboNum: $('comboNum'), reticle: $('reticle'),
  callout: $('callout'), menu: $('menu'), dead: $('dead'), deadStats: $('deadStats'), start: $('start'), restart: $('restart'),
  speedBar: $('speedBar'), flash: $('flash')
};
...
scene.fog = new THREE.FogExp2(0x8fd5a7, 0.0145);
...
const game = { running: false, dead: false, time: 0, bananas: 0, combo: 1, best: 0, lock: null, msgTimer: 0, lastVine: null, pulse: 0 };
const player = { pos: new THREE.Vector3(0, 8, 0), vel: new THREE.Vector3(0, 0, 24), attached: null, rope: 10, assist: 0, roll: 0, bob: 0, shake: 0 };

const groups = {
  world: new THREE.Group(), vines: new THREE.Group(), pickups: new THREE.Group(), hazards: new THREE.Group(), river: new THREE.Group(),
  fx: new THREE.Group(), arms: new THREE.Group(), canopy: new THREE.Group(), nearLeaves: new THREE.Group(), birds: new THREE.Group()
};
scene.add(groups.world, groups.vines, groups.pickups, groups.hazards, groups.river, groups.fx, groups.canopy, groups.nearLeaves, groups.birds);
...
const mats = {
  ...
  barkDark: new THREE.MeshStandardMaterial({ color: 0x342112, roughness: 1 }),
  ...
  rockWet: new THREE.MeshStandardMaterial({ color: 0x354b44, roughness: 0.72, metalness: 0.02 }),
  ...
  lock: new THREE.MeshBasicMaterial({ color: 0x72ff9a, transparent: true, opacity: 0.6, depthWrite: false }),
...
function makeLeafCluster(scale = 1, richness = 8) {
  ...
  for (let i = 0; i < richness; i++) {
    ...
    const o = new THREE.Mesh(new THREE.SphereGeometry(rand(1.2, 2.4) * scale, 8, 6), mat);
...
function makePalmFan(scale = 1) {
  const g = new THREE.Group();
  for (let i = 0; i < 7; i++) {
    const leaf = new THREE.Mesh(new THREE.PlaneGeometry(1.2 * scale, 5.5 * scale, 1, 1), mats.palm);
    leaf.position.y = 0.2;
...
const ropeLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(), new THREE.Vector3()]), new THREE.LineBasicMaterial({ color: 0xdfffd0, transparent: true, opacity: 0.86 }));
ropeLine.visible = false; scene.add(ropeLine);
...
function tree(x, z, big = false) {
  ...
  for (let i = 0; i < (big ? 3 : 1); i++) {
    const branch = cylinder(0.18, 0.28, rand(5, 11), 6, mats.barkDark);
...
function rock(x, z, s = 1) {
  const o = new THREE.Mesh(new THREE.DodecahedronGeometry(rand(2.5, 7) * s, 0), Math.random() < 0.28 ? mats.rockWet : mats.rock);
...
function ruin(z) {
  const g = new THREE.Group();
  for (let i = 0; i < 5; i++) { const c = cylinder(0.45, 0.62, rand(6, 12), 10, mats.ruin); c.position.set((i - 2) * 2.5, c.geometry.parameters.height / 2 - 5, rand(-1.1, 1.1)); g.add(c); }
  const lintel = new THREE.Mesh(new THREE.BoxGeometry(12, 1.15, 1.6), mats.ruin); lintel.position.y = 6.1; g.add(lintel);
  for (let i = 0; i < 7; i++) { const rubble = new THREE.Mesh(new THREE.BoxGeometry(rand(0.6, 1.8), rand(0.25, 0.9), rand(0.6, 1.8)), mats.ruin); rubble.position.set(rand(-7, 7), rand(-5.2, -3.8), rand(-3, 3)); rubble.rotation.set(rand(0, 1), rand(0, 6.28), rand(0, 1)); g.add(rubble); }
...
function vine(x, y, z) {
  ...
  const curve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(0, 0, 0), new THREE.Vector3(rand(-0.35, 0.35), -3.2, rand(-0.22, 0.22)),
    new THREE.Vector3(rand(-0.55, 0.55), -6.8, rand(-0.3, 0.3)), new THREE.Vector3(rand(-0.75, 0.75), -10.8, rand(-0.4, 0.4))
  ]);
  g.add(new THREE.Mesh(new THREE.TubeGeometry(curve, 22, 0.095, 8, false), mats.vine));
  const ridge = new THREE.Mesh(new THREE.TubeGeometry(curve, 22, 0.034, 5, false), mats.vineRidge); ridge.position.x = 0.08; g.add(ridge);
  for (let i = 0; i < 3; i++) { const lf = makeLeafCluster(0.35, 4); lf.position.set(rand(-0.6, 0.6), rand(-4.5, -1.1), rand(-0.25, 0.25)); lf.scale.setScalar(rand(0.6, 1.0)); g.add(lf); }
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.78, 0.045, 8, 36), mats.lock); ring.visible = false; ring.rotation.x = Math.PI / 2; ring.position.y = -0.45; g.add(ring);
  g.position.set(x, y, z); g.userData = { anchor: new THREE.Vector3(x, y, z), ring, sway: rand(0, Math.PI * 2) };
...
function canopyCurtain(z) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    lf.rotation.set(rand(-0.4, 0.4), side * rand(0.7, 1.25), rand(-0.9, 0.9));
    g.add(lf);
  }
  groups.canopy.add(g);
}
...
function section(z) {
  ...
  if (Math.random() < 0.86) vine(rand(-4.4, 4.4), rand(13, 17), z + rand(-1, 4));
...
function ensureWorld() {
  ...
  vines = vines.filter(o => o.parent && o.position.z > player.pos.z - 90); bananas = bananas.filter(o => o.parent && o.position.z > player.pos.z - 90); hazards = hazards.filter(o => o.parent && o.position.z > player.pos.z - 90);
}
...
function makeAtmosphere() {
  ...
  for (let i = 0; i < 26; i++) { const mist = new THREE.Mesh(new THREE.PlaneGeometry(rand(18, 46), rand(8, 18)), mats.mist.clone()); mist.material.opacity = rand(0.035, 0.095); mist.position.set(rand(-38, 38), rand(-6, 8), rand(20, 360)); mist.rotation.set(rand(-0.15, 0.15), rand(-0.3, 0.3), rand(-0.1, 0.1)); groups.fx.add(mist); }
  for (let i = 0; i < 18; i++) { const bird = new THREE.Mesh(new THREE.PlaneGeometry(rand(0.6, 1.2), rand(0.12, 0.22)), mats.bird); bird.position.set(rand(-45, 45), rand(18, 35), rand(40, 360)); bird.userData = { phase: rand(0, Math.PI * 2), speed: rand(0.6, 1.4) }; groups.birds.add(bird); }
}
makeAtmosphere();

function reset() {
  game.running = true; game.dead = false; game.time = 0; game.bananas = 0; game.combo = 1; game.lastVine = null; game.msgTimer = 0; game.pulse = 0;
  player.pos.set(0, 8, 0); player.vel.set(0, 0, 25); player.attached = null; player.rope = 10; player.assist = 0.8; player.shake = 0;
...
function lockVine() {
  ...
  for (const v of vines) { v.userData.ring.visible = false; if (v === game.lastVine) continue; const a = v.userData.anchor, to = new THREE.Vector3().subVectors(a, player.pos); if (to.z < 1.3 || to.z > 25) continue; const d = to.length(), grace = 5.5 + player.assist * 2.3; if (Math.abs(to.x) < 9.5 && to.y > 1 && d < player.rope + grace) { const s = Math.abs(to.z - 9.5) * 1.03 + Math.abs(to.x) * 1.62 + Math.abs(to.y - 5) * 0.35; if (s < score) { score = s; best = v; } } }
  if (best) best.userData.ring.visible = true; game.lock = best; ui.reticle.classList.toggle('lock', !!best); return best;
}
function attach(v) { if (!v || player.attached) return; player.attached = v; game.lastVine = v; player.rope = clamp(player.pos.distanceTo(v.userData.anchor), 7.2, 12.8); player.vel.z = Math.max(player.vel.z, 21); player.vel.y = Math.max(player.vel.y, -0.5); player.assist = 0.25; player.shake = 0.045; game.combo = clamp(game.combo + 0.4, 1, 10); navigator.vibrate && navigator.vibrate(10); }
function release() { if (!player.attached) return; const anchor = player.attached.userData.anchor; const forward = player.pos.z - anchor.z; const perfect = forward > player.rope * 0.45 && player.vel.y > -3; player.attached = null; player.vel.z = Math.max(player.vel.z, perfect ? 29 : 24) + (perfect ? 3.8 : 0) + game.combo * 0.4; player.vel.y += perfect ? 3.25 : 1.9; player.assist = perfect ? 1.15 : 0.78; player.shake = perfect ? 0.078 : 0.04; if (perfect) { game.combo = clamp(game.combo + 0.8, 1, 10); call('Perfect release'); flash(); navigator.vibrate && navigator.vibrate([8, 20, 12]); } }

function physics(dt) {
  input.steer = lerp(input.steer, input.target, 1 - Math.pow(0.001, dt)); player.assist = Math.max(0, player.assist - dt * 0.65);
  const target = lockVine(); if (input.hold && !player.attached && target) attach(target); if (!input.hold && player.attached) release();
  if (player.attached) { const a = player.attached.userData.anchor; player.vel.y -= 13.4 * dt; player.vel.x += input.steer * 11.8 * dt; player.vel.z += 5.0 * dt; player.pos.addScaledVector(player.vel, dt); let rope = new THREE.Vector3().subVectors(player.pos, a), len = rope.length(); if (len > player.rope) { const n = rope.normalize(); player.pos.copy(a).addScaledVector(n, player.rope); const radial = n.multiplyScalar(player.vel.dot(n)); player.vel.sub(radial.multiplyScalar(0.955)); player.vel.z += 0.75; } if (player.pos.z > a.z + player.rope * 0.88 || player.vel.z > 44) release(); }
  else { player.vel.y -= 16.9 * dt; player.vel.z = lerp(player.vel.z, 22, 0.38 * dt); player.vel.x += input.steer * 12.2 * dt; player.vel.x *= Math.pow(0.935, dt * 60); player.pos.addScaledVector(player.vel, dt); game.combo = Math.max(1, game.combo - dt * 0.22); if (player.pos.y < -3.2 || Math.abs(player.pos.x) > 14) die(); }
  player.pos.x = clamp(player.pos.x, -12, 12);
  for (const b of bananas) if (!b.userData.collected && b.position.distanceTo(player.pos) < 1.55) { b.userData.collected = true; groups.pickups.remove(b); game.bananas++; game.combo = clamp(game.combo + 0.55, 1, 10); call('+1 banana'); navigator.vibrate && navigator.vibrate(7); }
  for (const h of hazards) if (h.position.distanceTo(player.pos) < h.userData.radius + 0.55) die();
}

function updateCamera(dt) {
  player.bob += dt * (player.attached ? 8.5 : 10); const speed = player.vel.length(); camera.fov = lerp(camera.fov, clamp(76 + (speed - 20) * 0.58, 76, 92), 1 - Math.pow(0.02, dt)); camera.updateProjectionMatrix();
  player.roll = lerp(player.roll, clamp(-player.vel.x * 0.018 + input.steer * 0.08, -0.16, 0.16), 1 - Math.pow(0.02, dt)); player.shake = Math.max(0, player.shake - dt * 0.8);
  const jitter = player.shake ? new THREE.Vector3(rand(-player.shake, player.shake), rand(-player.shake, player.shake), 0) : new THREE.Vector3(); camera.position.copy(player.pos).add(jitter); camera.position.y += 0.66 + Math.sin(player.bob) * 0.035;
  camera.lookAt(player.pos.x + player.vel.x * 0.032 + input.steer * 1.15, player.pos.y + 0.35 + player.vel.y * 0.015, player.pos.z + 10.5); camera.rotateZ(player.roll);
  const grab = player.attached ? 1 : (input.hold ? 0.42 : 0), arms = groups.arms; arms.position.set(input.steer * 0.055, -0.025 + Math.sin(player.bob) * 0.006, -0.02); arms.rotation.x = lerp(arms.rotation.x, grab ? -0.2 : 0.08, 0.12); arms.userData.l.position.y = lerp(arms.userData.l.position.y, grab ? 0.13 : 0, 0.12); arms.userData.r.position.y = lerp(arms.userData.r.position.y, grab ? 0.13 : 0, 0.12); arms.userData.l.rotation.z = lerp(arms.userData.l.rotation.z, grab ? -0.28 : 0, 0.12); arms.userData.r.rotation.z = lerp(arms.userData.r.rotation.z, grab ? 0.28 : 0, 0.12);
  if (player.attached) { ropeLine.visible = true; ropeLine.geometry.setFromPoints([player.attached.userData.anchor.clone(), player.pos.clone().add(new THREE.Vector3(input.steer * 0.2, 0.1, -0.3))]); } else ropeLine.visible = false;
}

function visuals(dt) {
  const shafts = groups.fx.userData.shafts; if (shafts) { shafts.position.z = player.pos.z; shafts.position.x = player.pos.x * 0.25; }
  for (const v of vines) { v.rotation.z = Math.sin(game.time * 1.5 + v.userData.sway) * 0.07; v.rotation.x = Math.sin(game.time * 1.1 + v.userData.sway) * 0.03; v.userData.ring.rotation.z += dt * 1.8; }
  ...
  for (const b of groups.birds.children) { b.position.x += dt * b.userData.speed * 1.7; b.position.y += Math.sin(game.time * 6 + b.userData.phase) * 0.006; b.rotation.z = Math.sin(game.time * 9 + b.userData.phase) * 0.35; if (b.position.x > 55) b.position.x = -55; if (b.position.z < player.pos.z - 30) b.position.z = player.pos.z + rand(180, 390); }
  for (const m of groups.fx.children) if (m.material && m.material.opacity) m.position.z += Math.sin(game.time * 0.6 + m.position.x) * 0.002;
}

function updateUI(dt) {
  ui.dist.textContent = Math.max(0, Math.floor(player.pos.z)); ui.ban.textContent = game.bananas; ui.comboNum.textContent = Math.floor(game.combo); ui.combo.classList.toggle('show', game.combo >= 2); ui.speedBar.style.width = `${clamp((player.vel.length() - 12) / 38 * 100, 0, 100)}%`;
  if (game.msgTimer > 0) { game.msgTimer -= dt; if (game.msgTimer <= 0) ui.callout.classList.remove('show'); }
}

function loop() {
  const dt = Math.min(clock.getDelta(), 0.033); game.time += dt;
...
window.addEventListener('pointermove', e => move(e.clientX), { passive: true }); window.addEventListener('pointerup', up, { passive: true });
...
window.addEventListener('touchmove', e => { move(e.changedTouches[0].clientX); e.preventDefault(); }, { passive: false }); window.addEventListener('touchend', e => { up(); e.preventDefault(); }, { passive: false });
...
ui.start.addEventListener('click', reset); ui.restart.addEventListener('click', reset); ui.start.addEventListener('touchend', e => { e.preventDefault(); reset(); }, { passive: false }); ui.restart.addEventListener('touchend', e => { e.preventDefault(); reset(); }, { passive: false });
